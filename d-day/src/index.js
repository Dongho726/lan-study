...
const doYear = doFormDate.querySelector("#year");
const doMonth = doFormDate.querySelector("#month");
const doDay = doFormDate.querySelector("#day");
...
function getTime(paintName, paintYear, paintMonth, paintDay) {
  // Don't delete this.
  const xmasDay = new Date(`${paintYear}-${paintMonth}-${paintDay}:00:00:00`);
  const presentDay = new Date();
  const dDay = xmasDay.getTime() - presentDay.getTime();
  const dDate = Math.floor(dDay / 60 / 60 / 24 / 1000);
...
function handle(event) {
  ...
  paintToDay(handleName(), handleYear(), handleMonth(), handleDay());
}
